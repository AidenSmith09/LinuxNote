...
类型: Enforcing, Permissive, or Disabled 

配置文件 /etc/sysconfig/selinux 

图形工具 yum install policycoreutils-gui system-config-selinux 

运行时修改 getenforce and setenforce 0 \| 1 Kernel arguments: selinux=0 \| 1 or enforcing=0 \| 1
...
首先需要安装iptables的服务包 `yum install iptables-services` 因为RHEL7默认使用防火墙工具为firewalld

所以需要关闭firewalld才能开启iptables 
